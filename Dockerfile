...
RUN echo "deb [signed-by=/usr/share/keyrings/cloud.google.asc] https://packages.cloud.google.com/apt gcsfuse-$(env -i bash -c '. /etc/os-release; echo $VERSION_CODENAME') main" | sudo tee /etc/apt/sources.list.d/gcsfuse.list
